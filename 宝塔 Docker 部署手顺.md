...
#### 移动端适配
本项目已完整支持移动端访问，包含以下页面：

| 路由 | 功能 | 说明 |
|------|------|------|
| `/m/home` | 移动端首页 | 文章 Feed 流 |
| `/m/articles` | 文章列表 | 分类筛选 |
| `/m/article/:id` | 文章详情 | Markdown 渲染、图片显示 |
| `/m/ai` | AI 助手 | 图片上传、图文笔记 |
| `/m/categories` | 分类页面 | 分类列表 |
| `/m/editor/:id` | 编辑器 | 文章编辑 |
| `/m/profile` | 个人中心 | 个人信息修改 |
...
#### AI 助手增强
- **图片上传**: 支持上传图片（最大 5MB）
- **图文笔记**: AI 根据图片生成图文混排的读书笔记
- **自动保存**: 生成的文章自动保存到博客和知识库

#### 使用图文笔记功能
1. 移动端访问 `/m/ai`
2. 点击图片图标上传图片
3. 输入"请根据这张图片帮我写读书笔记"
4. AI 生成包含图片的 Markdown 内容
5. 文章自动保存，可在文章列表查看
